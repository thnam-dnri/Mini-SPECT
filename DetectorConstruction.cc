...
G4VPhysicalVolume* DetectorConstruction::Construct() {
    ...
    worldPV = new G4PVPlacement(
        nullptr,
        G4ThreeVector(),    // no translation: origin
        worldLV,
        "WorldPV",
    ...
    // ------------------------------------------------------------
    // 3) Multi‐organ phantom: body + thyroid at +20 cm (or fPhantomOffset)
    // ------------------------------------------------------------
    // 3a) Body (ellipsoid), centered at fPhantomOffset
    ...
    // 3b) Thyroid (smaller ellipsoid), same center as body
    ...
    const G4double cX = 3.16*mm, cY = 3.16*mm, cZ = 20*mm;  // crystal dimensions
    const G4double gap = 0.2*mm;                            // Teflon gap
    const G4double detX = nX*(cX + gap) - gap;              // total width in X
    const G4double detY = nY*(cY + gap) - gap;              // total width in Y

    // 4a) Teflon reflector: Big box minus "holes" where crystals sit
    ...
    // ------------------------------------------------------------
    // 5) Collimator: 10 cm lead block with 16 pinholes (1.68 mm diameter)
    // ------------------------------------------------------------
    G4double colTh = 5*cm;
    G4Box* colBase = new G4Box("CollimatorBase", detX/2, detY/2, colTh/2);
    ...
    // Subtract each pinhole from the lead block; also store its center
    G4VSolid* colS = colBase;
    fPinholeCenters.clear();  // ensure empty before we start
    for (G4int i = 0; i < nX; ++i) {
        for (G4int j = 0; j < nY; ++j) {
            G4double x = -detX/2 + cX/2 + i*(cX + gap);
            G4double y = -detY/2 + cY/2 + j*(cY + gap);

            // Store this pinhole center (in detector coords) for later use:
            fPinholeCenters.emplace_back(x, y, 0.0);

            colS = new G4SubtractionSolid(
                "CollimatorHoles",
                colS,
                pinS,
                nullptr,
    ...
    // ------------------------------------------------------------
    // 6) Crystals: place CsI(Tl) volumes into each "hole" of the reflector
    //    at z = –cZ/2 (so crystals sit behind the reflector)
    // ------------------------------------------------------------
