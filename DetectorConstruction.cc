...
G4VPhysicalVolume* DetectorConstruction::Construct() {
    ...
    worldPV = new G4PVPlacement(nullptr, G4ThreeVector(), worldLV, "WorldPV", nullptr, false, 0);
    ...
    new G4PVPlacement(nullptr, fPhantomOffset, bodyLV, "BodyPV", worldLV, false, 0);
    ...
    new G4PVPlacement(nullptr, fPhantomOffset, thyroidLV, "ThyroidPV", worldLV, false, 0);
    ...
    // Add small test box (radioactive, 30%) inside body, offset from thyroid
    ...
    new G4PVPlacement(nullptr, boxGlobalPos, testBoxLV, "TestBoxPV", worldLV, false, 0);
    ...
    new G4PVPlacement(nullptr, G4ThreeVector(0, 0, -cZ/2), reflLV, "ReflectorPV", worldLV, false, 0);
    ...
    G4Tubs* pinS = new G4Tubs("Pinhole", 0.0, pinholeRadius, colTh/2 + 1*mm, 0.0, 360.0*deg);
    ...
    new G4PVPlacement(nullptr, G4ThreeVector(0, 0, +colTh/2), colLV, "CollimatorPV", worldLV, false, 0);
    ...
    for (G4int i = 0; i < nX; ++i) {
        for (G4int j = 0; j < nY; ++j) {
            G4double x = -detX/2 + cX/2 + i*(cX + gap);
            G4double y = -detY/2 + cY/2 + j*(cY + gap);
            new G4PVPlacement(nullptr, G4ThreeVector(x, y, -cZ/2), crystalLV, "CrystalPV", worldLV, false, i * nY + j);
        }
    }
